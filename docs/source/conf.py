    ...
    if any(
        ...
        [
            ...
            for x in [
                ...
                "Installation",
                ".. |DefaultAugmentation|",
                ".. figure:: images/logo/rising_logo.png",
            ]
        ]
    ):
        skip = False
...
for item in os.listdir(nb_path):
    if os.path.isfile(os.path.join(nb_path, item)) and item.endswith(".ipynb"):
        shutil.copy2(os.path.join(nb_path, item), os.path.join(nb_doc_path, item))
...
MOCK_MANUAL_PACKAGES = ["torch", "torchvision", "numpy", "dill"]
